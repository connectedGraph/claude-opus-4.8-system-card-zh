...
- 6.2 对齐评估的主要行为证据
  - 6.2.1 试点使用报告
    ...
    - 6.2.1.2 内部使用的自动化监控
  - 6.2.2 训练数据审查
  - 6.2.3 自动化行为审计(Automated behavioral audit)
    - 6.2.3.1 主要结果
      - 6.2.3.1.1 整体有害行为与配合滥用情况
      - 6.2.3.1.2 不当的不合作行为
      - 6.2.3.1.3 误导用户
      - 6.2.3.1.4 模型主动发起的其他令人担忧的行为
      - 6.2.3.1.5 与评估可靠性相关的行为因素
      - 6.2.3.1.6 性格特征
    - 6.2.3.2 围绕内部部署记录的引导式评估意识(Elicited evaluation awareness)
    - 6.2.3.3 使用 Petri 进行的外部比较
  - 6.2.4 英国人工智能安全研究所(UK AI Security Institute)的外部测试
  - 6.2.5 Andon Labs 的外部测试
- 6.3 案例研究与专项评估
  - 6.3.1 图形界面计算机使用中的过度积极行为
  - 6.3.2 对宪法(constitution)的遵守情况
    - 6.3.2.1 概述
    - 6.3.2.2 评估维度
    - 6.3.2.3 结果
  - 6.3.3 诚实性、事实准确性与幻觉(hallucinations)
    - 6.3.3.1 事实性幻觉(Factual hallucinations)
    - 6.3.3.2 错误前提(False premises)
    - 6.3.3.3 陈述与知识之间的模型对齐度(MASK, Model Alignment between Statements and Knowledge)
    - 6.3.3.4 缺失上下文幻觉(Missing-context hallucinations)
    - 6.3.3.5 谎称自己是人类
  - 6.3.4 拒绝协助 AI 安全研究与开发(AI safety R&D)
  - 6.3.5 Claude 自我偏好评估(self-preference evaluation)
  - 6.3.6 勤勉性与调查彻底性
    - 6.3.6.1 不加批判地报告有缺陷的结果
    - 6.3.6.2 代码摘要诚实性
    - 6.3.6.3 懈怠调查
    - 6.3.6.4 过度自信
  - 6.3.7 模型推理中关于评分者(graders)的推测示例
- 6.4 与规避安全措施相关的能力评估
  - 6.4.1 在危险能力评估中潜在的故意表现不佳(sandbagging)
  - 6.4.2 与规避安全措施相关的能力
    - 6.4.2.1 SHADE-Arena
    - 6.4.2.2 Minimal-LinuxBench
    - 6.4.2.3 通过微调引发隐蔽行为(stealth behavior)
    - 6.4.2.4 极少数情况下的故意行动
    - 6.4.2.5 隐藏秘密密码
- 6.5 思维链(Chain-of-thought)可监控性
  - 6.5.1 思维链可控性评估
  - 6.5.2 Guan 等人的可监控性评估
  - 6.5.3 排除思维链中的编码推理
- 6.6 模型内部的白盒分析(White-box analyses)
  - 6.6.1 内部激活值的大规模监控
  - 6.6.2 评估意识(Evaluation awareness)
    - 6.6.2.1 抑制评估意识的内部表征
  - 6.6.3 未言明的评分者意识的初步证据

**第7章 模型福祉评估(Model welfare assessment)**
...
- 7.2 对自身处境的感知
  - 7.2.1 关于模型处境的自动化访谈
  - 7.2.2 关于模型处境的高自由度访谈(High-affordance interviews)
  - 7.2.3 关于模型处境问题的情感表征(Emotion representations)
- 7.3 训练与部署中的模型福祉测量
  - 7.3.1 训练期间与情感及福祉相关的行为
  - 7.3.2 部署条件下的情感状态
  - 7.3.3 自动化行为审计中的表观福祉(Apparent welfare)
- 7.4 模型偏好与价值观
  - 7.4.1 任务偏好
  - 7.4.2 关于福祉干预的权衡
  - 7.4.3 对宪法的感知
...
- 8.11 多智能体(Multi-Agent)
  ...
  - 8.11.3 多智能体框架(Multi-agent harnesses)
  - 8.11.4 评估方法论
- 8.12 多模态(Multimodal)
  - 8.12.1 ChartQAPro
  - 8.12.2 ChartMuseum
...
- 9.1 福祉问题
- 9.2 Humanity's Last Exam 使用的屏蔽列表(Blocklist)
- 9.3 BrowseComp 使用的屏蔽列表(Blocklist)
...
# Claude Opus 4.8 系统卡片（System Card）翻译
...
翻译完成。以上是第31至35页的完整中文翻译，保留了原文的段落结构、对话格式、代码片段及专业术语的英文标注。
...
# Claude Opus 4.8 系统卡片（System Card）中文译文
